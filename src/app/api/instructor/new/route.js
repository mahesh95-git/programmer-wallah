...
export async function POST(req) {
  try {
    ...
    if (user.isApprovedInstructor === "pending") {
      return NextResponse.json(
        { message: "User is still pending for approval", success: false },
        { status: 400 }
      );
    }
    if (user.isApprovedInstructor === "approved") {
      return NextResponse.json(
        { message: "User is already approved", success: false },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      !contactNumber ||
      !expertise ||
      (!qualificationcCertificate && qualificationcCertificate == "null")
    ) {
      return NextResponse.json(
    ...
    fileUpload.push({
      file: qualificationcCertificate,
      subType: "qualificationcCertificate",
    });
    ...
    return NextResponse.json(
      {
        message:
          "Your application has been successfully submitted. We will reply to you within 24 hours.",
        success: true,
      },
      { status: 200 }
    );
    ...
    return NextResponse.json(
      { message: "Failed to submit data", success: false },
      { status: 500 }
    );
  }
}
